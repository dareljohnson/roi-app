...
const testJsonExport = {
  users: [
    { id: 'test-user', email: 'test@example.com', name: 'Test User', role: 'ADMIN' }
  ],
  properties: [],
  analyses: [],
  accounts: [],
...
describe('Admin DB Restore API (JSON)', () => {
  let req: NextRequest;
  ...
  it('should reject non-admins', async () => {
    ...
    req = new NextRequest('http://localhost:3000/api/admin/restore', {
      method: 'POST',
      body: JSON.stringify({ jsonExport: testJsonExport }),
  ...
  it('should allow admin to restore from JSON', async () => {
    // Mock admin session
